...
def PublicTest(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(PublicTestloader):
        ...
        loss1 = nn.CrossEntropyLoss()(outputs_avg,targets)
        loss1.backward()
        ...
        utils.progress_bar(batch_idx, len(PublicTestloader), 'Loss: %.3f | Acc: %.3f%% (%d/%d)'% (PublicTest_loss / (batch_idx + 1), 100. * correct / total, correct, total))


        inputs1 = inputs +  100000000.*data_grad
        #print(10000000*data_grad)
        #print(inputs1)

        outputs1 = net(inputs1)
        outputs1_avg = outputs1.view(bs,ncrops,-1).mean(1)
        loss2 = criterion(outputs1_avg,targets)
        PublicTest_loss1 += loss2.data
        _,predicted = torch.max(outputs_avg.data,1)
        correct1 += predicted.eq(targets.data).cpu().sum()
    ...
    PublicTest_acc1 = 100.*correct1/total
    print("accuracy: %0.3f" % PublicTest_acc)
    print("accuracy1: %0.3f" % PublicTest_acc1)
